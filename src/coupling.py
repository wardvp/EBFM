...
class YACCoupler(Coupler):
    ...
    def construct_coupling_elmer_ice(self, time: Dict[str, float]):
        ...
        field_definitions = [
            FieldDefinition(exchange_type=yac.ExchangeType.SOURCE, name="T_ice", metadata="Near surface temperature at Ice surface (in K)"),
        ...
        timestep = Timestep(value=timestep_value,
                            format=yac.TimeUnit.ISO_FORMAT)

        for field_definition in field_definitions:
            field = yac.Field.create(field_definition.name, self.component, self.corner_points, collection_size, timestep.value, timestep.format)
            self.interface.def_field_metadata(field.component_name, field.grid_name, field_definition.name, field_definition.metadata.encode("utf-8"));
    ...
    def construct_coupling_icon_atmo(self, time: Dict[str, float]):
        ...
        field_definitions = [
            ...
            FieldDefinition(exchange_type=yac.ExchangeType.TARGET, name='pr', metadata='Precipitation rate (in kg m-2 s-1)'),
            FieldDefinition(exchange_type=yac.ExchangeType.TARGET, name='pr_snow', metadata='Precipitation rate of snow (in kg m-2 s-1)'),
            FieldDefinition(exchange_type=yac.ExchangeType.TARGET, name='rsds', metadata='Downward shortwave radiation flux (in W m-2)'),
            FieldDefinition(exchange_type=yac.ExchangeType.TARGET, name='rlds', metadata='Downward longwave radiation flux (in W m-2)'),
            FieldDefinition(exchange_type=yac.ExchangeType.TARGET, name='sfcwind', metadata='Wind speed at surface (in m s-1)'),
    ...
    def construct_coupling_post_sync(self):
        ...
        for field in self.target_fields.values():
            ...
            assert(is_defined), f"Field '{field.name}' is not defined in YAC for component '{field.component_name}' and grid '{field.grid_name}'."
            field_role = self.interface.get_field_role(field.component_name, field.grid_name, field.name)
            assert(field_role == yac.ExchangeType.TARGET), f"Field '{field.name}' is no TARGET as expected but a {field_role}."
            field_info = self.field_information(field)
            logger.info(field_info)


    def field_information(self, field: yac.Field) -> str:
        src_comp, src_grid, src_field = self.interface.get_field_source(field.component_name, field.grid_name, field.name)
        ...
        return field_template.format(name=field.name, comp=src_comp, grid=src_grid, timestep=src_field_timestep, metadata=src_field_metadata)
    ...
    def exchange_icon_atmo(self, put_data: Dict[str, np.array]) -> Dict[str, np.array]:
        ...
        for field_name, field in self.source_fields.items():
            ...
            if field_name in icon_fields:
                ...
                assert role == yac.ExchangeType.SOURCE, f"Field '{field_name}' is not a target field for ICON exchange, but has role '{role}'."
        ...
        for field_name, field in self.target_fields.items():
            ...
            if field_name in icon_fields:
                role = self.interface.get_field_role(field.component_name, field.grid_name, field.name)
                assert role == yac.ExchangeType.TARGET, f"Field '{field_name}' is not a target field for ICON exchange, but has role '{role}'."
    ...
    def exchange_elmer_ice(self, put_data: Dict[str, np.array]) -> Dict[str, np.array]:
        ...
        for field_name, field in self.source_fields.items():
            if field_name in elmer_fields:
                role = self.interface.get_field_role(field.component_name, field.grid_name, field.name)
                assert role == yac.ExchangeType.SOURCE, f"Field '{field_name}' is not a target field for Elmer/Ice exchange, but has role '{role}'."
                self._put(field_name, put_data[field_name])
        ...
        for field_name, field in self.target_fields.items():
            if field_name in elmer_fields:
                role = self.interface.get_field_role(field.component_name, field.grid_name, field.name)
                assert role == yac.ExchangeType.TARGET, f"Field '{field_name}' is not a target field for Elmer/Ice exchange, but has role '{role}'."
                # Only get the first element, since we only have collection size of 1
                received_data[field_name] = self._get(field_name)[0]
