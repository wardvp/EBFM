...
def main(grid, time2, OUT):
    ...
    day_of_year = (time2["TCUR"] - start_of_year).days + (
            time2["TCUR"] - start_of_year).seconds / 86400  # Fractional days
    ...
    d = (0.322003
         ...
         + 0.05928 * np.sin(3.0 * trad))  # SOURCE: Iqbal (1983)

    # Solar hour angle (h)
    B = 360 / 365 * (day_of_year - 81)
    Tcor_ecc = 9.87 * np.sin(2.0 * np.radians(B)) - 7.53 * np.cos(np.radians(B)) - 1.5 * np.sin(
        np.radians(B))  # Correction for eccentricity
