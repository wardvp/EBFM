...
def parse_header(header_file: Path) -> tuple[int, int]:
    ...
    if df.shape[1] != n_expected_columns:
        raise ValueError(f"The header file has only {df.shape[1]} columns, but it should have exactly {n_expected_columns}.")
    
    if isinstance(df.iloc[0, 0], np.int_):
        n_vertices: int = int(df.iloc[0, 0])
    else:
        raise ValueError(f"Expected the first column of the header file to contain an integer, but got {type(df.iloc[0, 0])}.")
    
    if isinstance(df.iloc[0, 1], np.int_):
        n_cells: int = int(df.iloc[0, 1])
    else:
        raise ValueError(f"Expected the second column of the header file to contain an integer, but got {type(df.iloc[0, 1])}.")

    # Only the first row of the header file is relevant for the counts; other rows will be ignored.
    
    return n_vertices, n_cells

def parse_nodes(nodes_file: Path) -> tuple[NDArray[np.int_], NDArray[np.float64], NDArray[np.float64], NDArray[np.float64]]:
    ...
    if df.shape[1] < n_expected_columns:
        raise ValueError(f"The nodes file has only {df.shape[1]} columns, but it should have exactly {n_expected_columns}.")
...
def parse_elements(elements_file: Path) -> tuple[NDArray[np.int_], NDArray[np.int_]]:
    ...
    if df.shape[1] < n_expected_columns:
        raise ValueError(f"The elements file has only {df.shape[1]} columns, but it should have at least {n_expected_columns}.")
