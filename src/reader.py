...
def read_elmer_mesh(mesh_root: Path, is_partitioned: bool=False, rank: int=-1) -> TriangleMesh:
    ...
    assert header_file.is_file(), f"Header file {header_file} does not exist. Please ensure that this file exists in {mesh_root}."
    assert nodes_file.is_file(), f"Nodes file {nodes_file} does not exist. Please ensure that this file exists in {mesh_root}."
    assert elements_file.is_file(), f"Mesh file {elements_file} does not exist. Please ensure that this file exists in {mesh_root}."
    ...
    assert len(global_vertex_ids) == n_vertices, f"Number of vertex IDs in nodes file ({len(global_vertex_ids)}) does not match the header ({n_vertices})."
    assert len(x_vertices) == n_vertices, f"Number of vertices in nodes file ({len(x_vertices)}) does not match the header ({n_vertices})."
    assert len(y_vertices) == n_vertices, f"Number of vertices in nodes file ({len(y_vertices)}) does not match the header ({n_vertices})."
    assert len(z_vertices) == n_vertices, f"Number of vertices in nodes file ({len(z_vertices)}) does not match the header ({n_vertices})."
    ...
    vertex_l2g = {l:g for l,g in zip(local_vertex_ids, global_vertex_ids)}  # map local ids [0,1,...n_verts] to global_vertex_ids
    ...
    assert len(global_cell_ids) == n_cells, f"Number of cell IDs in elements file ({len(global_cell_ids)}) does not match the header ({n_cells})."
...
def read_dem(dem_file: Path, xs: NDArray[np.float64], ys: NDArray[np.float64]):
    ...
    print(f"{np.sum(abs(xs - nc['x'][idx_x][:]) > 10e-5)} of {len(idx_x)} x-coordinates have a significant mismatch. surface value of a neighboring point will be used.")
    print(f"{np.sum(abs(ys - nc['y'][idx_y][:]) > 10e-5)} of {len(idx_y)} y-coordinates have a significant mismatch. surface value of a neighboring point will be used.")
...
def write_dem_as_elmer(mesh: TriangleMesh, h: NDArray[np.float64], dem_file: Path, allow_overwrite: bool=False) -> None:
    ...
    assert len(h) == len(mesh.x_vertices), f"Height data length ({len(h)}) does not match number of vertices ({len(mesh.x_vertices)})."
