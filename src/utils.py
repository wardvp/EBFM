...
def setup_logging(log_levels=default_levels, file=None):
    ...
    log_formatter = logging.Formatter(fmt=f'EBFM r{MPI.COMM_WORLD.rank} ' + '%(asctime)s.%(msecs)03d %(levelname)s %(module)s - %(funcName)s: %(message)s',
                                    datefmt='%Y-%m-%d %H:%M:%S')

    if file:
        file_handler = logging.FileHandler(filename=f'{file}' + f'.{MPI.COMM_WORLD.rank}' if MPI.COMM_WORLD.size > 1 else '')
